use lib_ai::agent::memory::{Memory, SurrealMemoryStore, SurrealMemoryConfig};
...
async fn test_surrealdb_memory_initialization() {
    ...
    // Create memory store
    let memory = SurrealMemoryStore::new(config, embedding_provider).await.unwrap();
...
async fn test_surrealdb_memory_store_and_retrieve() {
    ...
    let mut memory = SurrealMemoryStore::new(config, embedding_provider).await.unwrap();
...
async fn test_surrealdb_memory_vector_search() {
    ...
    let conversations = vec![
        ("Tell me about machine learning", "Machine learning is a subset of artificial intelligence"),
        ("What's the weather like today?", "I don't have access to current weather data"),
    ...
    assert!(results[0].contains("machine learning") || results[0].contains("artificial intelligence") || results[0].contains("neural"));
}
...
async fn test_surrealdb_memory_clear() {
    ...
    // Store some entries
    for i in 0..5 {
        memory.store(&format!("Test message {}", i), &format!("Response {}", i)).await.unwrap();
    }
...
async fn test_surrealdb_memory_persistence() {
    ...
    {
        let mut memory = SurrealMemoryStore::new(config.clone(), Box::new(MockEmbeddingProvider::new(384))).await.unwrap();
        memory.store("What is Rust?", "Rust is a systems programming language").await.unwrap();
    }
