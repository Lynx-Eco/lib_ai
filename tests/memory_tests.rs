use lib_ai::agent::memory::{InMemoryStore, Memory, SemanticMemoryBuilder};
...
async fn test_in_memory_store() {
    ...
    memory
        .store("What's your name?", "I'm an AI assistant")
        .await
        .unwrap();
...
async fn test_semantic_memory() {
    ...
    let has_programming_content = results
        .iter()
        .any(|r| r.contains("Rust") || r.contains("Python") || r.contains("JavaScript"));
    assert!(
        has_programming_content,
        "Expected at least one programming-related result"
    );
...
async fn test_semantic_memory_similarity_threshold() {
    ...
    // Store an entry
    memory
        .store("The weather is nice today", "It's sunny and warm")
        .await
        .unwrap();
...
async fn test_semantic_memory_max_entries() {
    ...
    for i in 0..5 {
        memory
            .store(&format!("Message {}", i), &format!("Response {}", i))
            .await
            .unwrap();
    }
