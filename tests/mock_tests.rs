...
async fn test_openai_error_response() {
    ...
    let provider =
        lib_ai::providers::OpenAIProvider::with_base_url("test-key".to_string(), server.url());
...
async fn test_openai_success_response() {
    ...
    assert_eq!(
        response.choices[0].message.content.as_text().unwrap(),
        "Hello, World!"
    );
...
async fn test_streaming_response() {
    ...
    assert!(
        !full_content.is_empty() || chunks > 0,
        "Should have received some content or chunks"
    );
...
async fn test_connection_error() {
    ...
    assert!(result.is_err());
    match result {
        Err(AiError::NetworkError { .. }) | Err(AiError::ConnectionRefused { .. }) => {
            // Expected error type
        }
