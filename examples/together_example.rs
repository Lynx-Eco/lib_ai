...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api_key = env::var("TOGETHER_API_KEY")
        .expect("Please set TOGETHER_API_KEY environment variable");
    
    // Create Together provider
    let provider = TogetherProvider::new(api_key);
    ...
    let stream_request = CompletionRequest {
        ...
        messages: vec![
            Message {
                role: Role::User,
                content: MessageContent::text("Explain the concept of machine learning in simple terms"),
                tool_calls: None,
                tool_call_id: None,
            },
        ],
