use lib_ai::{
    agent::{AgentBuilder, CodeExecutorTool, FileSystemTool, HttpTool},
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let response = agent
        .execute("Create a file called 'test.txt' with the content 'Hello from AI Agent!'")
        .await?;
    ...
    println!("\n=== Code Execution ===");
    let response = agent
        .execute("Write and execute a Python script that calculates the first 10 Fibonacci numbers")
        .await?;
    println!("Agent: {}", response);
