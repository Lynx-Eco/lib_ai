use lib_ai::{
    ...
    error::{ResilientProvider, ResilientProviderBuilder, RetryConfig, CircuitBreakerConfig},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let custom_resilient = ResilientProvider::with_config(
        base_provider.clone(),
        retry_config,
        circuit_config,
    );
    ...
    if let Some(text) = result.choices[0].message.content.as_text() {
        println!("Final response: {}", text.chars().take(100).collect::<String>());
    }
