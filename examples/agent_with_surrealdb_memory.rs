use lib_ai::agent::memory::{SurrealMemoryConfig, SurrealMemoryStore};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut agent = AgentBuilder::new()
        .provider(provider)
        .prompt(
            "You are a helpful assistant with the ability to remember past conversations. \
    ...
    println!(
        "The agent's memory is persisted in SurrealDB and will be available in future sessions."
    );
