...
use lib_ai::{
    agent::{tools::FunctionTool, AgentBuilder, ToolExecutor, ToolResult},
    ...
    async fn execute(&self, arguments: &str) -> Result<ToolResult, Box<dyn std::error::Error>> {
        ...
        if num_dice == 0 || num_dice > 100 {
            return Ok(ToolResult::Error(
                "Number of dice must be between 1 and 100".to_string(),
            ));
        }

        if sides < 2 || sides > 1000 {
            return Ok(ToolResult::Error(
                "Number of sides must be between 2 and 1000".to_string(),
            ));
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = agent
        .execute("What's the current time, and roll 2d20 for me")
        .await?;
