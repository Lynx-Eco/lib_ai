...
impl ToolExecutor for CalculatorTool {
    async fn execute(&self, arguments: &str) -> Result<ToolResult, Box<dyn std::error::Error>> {
        ...
        let result = match input.operation.as_str() {
            ...
            _ => return Ok(ToolResult::Error(format!("Unknown operation: {}", input.operation))),
        };
...
impl ToolExecutor for TimezoneTool {
    async fn execute(&self, arguments: &str) -> Result<ToolResult, Box<dyn std::error::Error>> {
        ...
        let offset = FixedOffset::east_opt(input.timezone_offset * 3600)
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let response = agent.execute(
        "Calculate 24 * 7 and tell me what time it is in New York (UTC-5)"
    ).await?;
    println!("Response: {}", response);
