...
impl SurrealMemoryStore {
    ...
    pub async fn new(
        ...
        let db = Surreal::new::<Ws>(&config.url)
            .await
            .map_err(|e| AgentError::MemoryError(format!("Failed to connect to SurrealDB: {}", e)))?;
        ...
        db.use_ns(&config.namespace)
            ...
            .await
            .map_err(|e| AgentError::MemoryError(format!("Failed to select namespace/database: {}", e)))?;
        ...
        let create_table_query = format!(
            ...
            "#,
            config.table, config.table, config.table, config.table, config.table, config.table, config.table
        );
    ...
    async fn find_similar(
        ...
        let query = format!("SELECT * FROM {} ORDER BY created_at DESC LIMIT 1000", self.config.table);
...
impl Memory for SurrealMemoryStore {
    ...
    async fn retrieve(&self, query: &str, limit: usize) -> Result<Vec<String>, AgentError> {
        ...
        let embedding = self.embedding_provider
            .embed_single(query)
            .await
            .map_err(|e| AgentError::MemoryError(format!("Failed to generate query embedding: {}", e)))?;
