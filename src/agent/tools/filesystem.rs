...
impl ToolExecutor for FileSystemTool {
    async fn execute(&self, arguments: &str) -> Result<ToolResult, Box<dyn std::error::Error>> {
        ...
        let operation: FileOperation = serde_json::from_value(input)
        ...
        match operation {
            ...
            FileOperation::Delete { path } => {
                if !self.allow_write {
                    return Ok(ToolResult::Error("Delete operations are not allowed".to_string()));
                }
            ...
            FileOperation::Exists { path } => {
                let full_path = match self.resolve_path(&path) {
                    Ok(p) => p,
                    Err(_) => return Ok(ToolResult::Success(serde_json::json!({ "exists": false }))),
                };
            ...
            FileOperation::CreateDir { path } => {
                if !self.allow_write {
                    return Ok(ToolResult::Error("Create directory operations are not allowed".to_string()));
                }
