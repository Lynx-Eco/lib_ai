...
impl CostTracker {
    ...
    pub fn record_usage(
        ...
        let provider_costs = self.provider_costs.entry(provider.to_string()).or_insert_with(|| {
            ProviderCosts {
        ...
        // Update model costs
        let model_costs = provider_costs.models.entry(model.to_string()).or_insert_with(|| {
            ModelCosts {
    ...
    pub fn generate_report(&self) -> CostReport {
            ...
            for (model_name, model_costs) in &provider_costs.models {
                model_breakdown.push(ModelReportEntry {
                    ...
                    cost_per_token: model_costs.total_cost / 
                        (model_costs.input_tokens + model_costs.output_tokens + 
                         model_costs.cache_read_tokens + model_costs.cache_write_tokens).max(1) as f64,
                });
            }
        ...
        CostReport {
            ...
            duration: self.last_updated.signed_duration_since(self.start_time).to_std().unwrap_or_default(),
...
pub fn get_default_pricing() -> HashMap<String, PricingInfo> {
    ...
    // OpenAI GPT-4o
    pricing.insert("openai:gpt-4o".to_string(), PricingInfo {
    ...
    // OpenAI GPT-4o-mini
    pricing.insert("openai:gpt-4o-mini".to_string(), PricingInfo {
        provider: "openai".to_string(),
    ...
    pricing.insert("anthropic:claude-3-5-sonnet-20241022".to_string(), PricingInfo {
    ...
    pricing.insert("anthropic:claude-3-5-haiku-20241022".to_string(), PricingInfo {
        provider: "anthropic".to_string(),
    ...
    pricing.insert("google:gemini-1.5-pro".to_string(), PricingInfo {
...
mod tests {
    ...
    fn test_cost_report() {
        ...
        // Record some usage
        tracker.record_usage("openai", "gpt-4o", 1000, 500, 0, 0, 
                           pricing.get("openai:gpt-4o").unwrap());
        tracker.record_usage("anthropic", "claude-3-5-sonnet-20241022", 800, 300, 100, 50,
