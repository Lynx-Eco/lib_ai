...
impl MetricsCollector {
    ...
    pub fn record_request(&self, agent_id: &str, success: bool, duration: Duration, tokens: TokenUsage, cost: f64, provider: &str, model: &str) {
        let mut metrics = self.metrics.write().unwrap();
        if let Some(agent_metrics) = metrics.get_mut(agent_id) {
            ...
            let total_duration = agent_metrics.average_response_time * agent_metrics.total_requests as u32 + duration;
            agent_metrics.average_response_time = total_duration / agent_metrics.total_requests as u32;
            
            // Update provider metrics
            let provider_key = format!("{}:{}", provider, model);
            let provider_metrics = agent_metrics.provider_metrics.entry(provider_key).or_insert_with(|| {
                ProviderMetrics {
            ...
            provider_metrics.average_latency = provider_metrics.total_duration / provider_metrics.requests as u32;
        ...
        global.uptime = Utc::now().signed_duration_since(global.start_time).to_std().unwrap_or_default();
    }
    
    pub fn record_tool_execution(&self, agent_id: &str, tool_name: &str, success: bool, duration: Duration, error_type: Option<String>) {
        let mut metrics = self.metrics.write().unwrap();
        if let Some(agent_metrics) = metrics.get_mut(agent_id) {
            let tool_metrics = agent_metrics.tool_usage.entry(tool_name.to_string()).or_insert_with(|| {
                ToolMetrics {
            ...
            tool_metrics.average_duration = tool_metrics.total_duration / tool_metrics.executions as u32;
